...
If Object_ID('dbo.p_Cities_Get','P') Is Null
    Exec('Create Procedure dbo.p_Cities_Get As Select 1')
Go
/*
	Revision History:
		2/27/2019	Created
	Example Query:
		...
		Exec dbo.p_Cities_Get @UserID = 1, @RetVal = @RetVal out, @RetMsg = @RetMsg out
		Select @RetVal, @RetMsg
*/

Alter Procedure dbo.p_Cities_Get
(
	@UserID Int,
...
Select l.CountryID, 1.StateID, l.CityID, l.Name, l.Abbrev, l.Disabled,
...
From dbo.t_Cities As l With (NoLock)
...
And l.StateID In (0, @StateID)
And l.CityID In (0, @CityID)
