...
If Object_ID('dbo.p_States_Set','P') Is Null
    Exec('Create Procedure dbo.p_States_Set As Select 1')
Go
/*
	Revision History:
		2/27/2019	Created
	Example Query:
		...
		Exec dbo.p_States_Set @UserID = 1, @RetVal = @RetVal out, @RetMsg = @RetMsg out
		Select @RetVal, @RetMsg
*/

Alter Procedure dbo.p_States_Set
(
	@UserID Int,
...
Select @StateID = IsNull(@StateID, 0),
...
Begin
	...
	Insert Into dbo.t_States
	(CountryID, StateID, Name, Abbrev, Disabled, CreatedID, CreatedDateTime)
	Values
	(@CountryID, @StateID, @Name, @Abbrev, 0, @UserID, @dUtcNow)
...
Begin
	...
	Update dbo.t_States
	Set CountryID = @CountryID, StateID = @StateID, Name = @CountryID, Abbrev = @Abbrev, Disabled = @Disable,
