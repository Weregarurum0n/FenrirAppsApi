...
If Object_ID('dbo.p_Cities_Set','P') Is Null
    Exec('Create Procedure dbo.p_Cities_Set As Select 1')
Go
/*
	Revision History:
		2/27/2019	Created
	Example Query:
		...
		Exec dbo.p_Cities_Set @UserID = 1, @RetVal = @RetVal out, @RetMsg = @RetMsg out
		Select @RetVal, @RetMsg
*/

Alter Procedure dbo.p_Cities_Set
(
	@UserID Int,
...
Begin
	...
	Insert Into dbo.t_Cities
	(CountryID, StateID, CityID, Name, Abbrev, Disabled, CreatedID, CreatedDateTime)
	Values
	(@CountryID, @StateID, @CityID, @Name, @Abbrev, 0, @UserID, @dUtcNow)
...
Begin
	...
	Update dbo.t_Cities
	Set CountryID = @CountryID, StateID = @StateID, CityID = @CityID, Name = @CountryID, Abbrev = @Abbrev, Disabled = @Disable,
