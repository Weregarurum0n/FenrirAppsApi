...
If Object_ID('dbo.p_Katakana_Set','P') Is Null
    Exec('Create Procedure dbo.p_Katakana_Set As Select 1')
Go
/*
	Revision History:
		Date		Editor					Comment
	...
	Example Query:
		...
		Exec dbo.p_Katakana_Set @UserID = 1, @RetVal = @RetVal out, @RetMsg = @RetMsg out
		Select @RetVal, @RetMsg
*/

Alter Procedure dbo.p_Katakana_Set
(
	@UserID Int,
...
If (@KatakanaID = 0)
Begin
	Select @KatakanaID = Max(KatakanaID) + 10 From dbo.t_Katakana With (NoLock)

	Insert Into dbo.t_Katakana
