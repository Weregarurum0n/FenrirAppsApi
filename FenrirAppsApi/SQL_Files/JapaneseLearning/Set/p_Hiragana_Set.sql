...
If Object_ID('dbo.p_Hiragana_Set','P') Is Null
    Exec('Create Procedure dbo.p_Hiragana_Set As Select 1')
Go
/*
	Revision History:
		Date		Editor					Comment
	...
	Example Query:
		...
		Exec dbo.p_Hiragana_Set @UserID = 1, @RetVal = @RetVal out, @RetMsg = @RetMsg out
		Select @RetVal, @RetMsg
*/

Alter Procedure dbo.p_Hiragana_Set
(
	@UserID Int,
...
If (@HiraganaID = 0)
Begin
	Select @HiraganaID = Max(HiraganaID) + 10 From dbo.t_Hiragana With (NoLock)

	Insert Into dbo.t_Hiragana
